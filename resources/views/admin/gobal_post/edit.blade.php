                ...
                <div class="statbox widget box box-shadow mb-3">
                    ...
                    <div class="widget-content widget-content-area text-center">
                        ...
                        <div class="row">
                                ...
                                <nav class="breadcrumb-two" aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active"><a
                                                href="{{ route('admin.post', $postType->slug) }}">{{ $postType->title }}</a>
                                        </li>
                            ...
                            <div class="col-sm-6">
                                <a href="{{ route('admin.post', $postType->slug) }}"
                                    class="btn btn-sm btn-success float-right">Back to {{ $postType->title }} List</a>
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-12 mx-auto">
                            <form method="post" enctype="multipart/form-data"
                                action="{{ route('admin.post.update', $postType->slug) }}">
                                {{ csrf_field() }}
                                ...
                                <div class="row">
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            ...
                                            <input id="post_type_title" value="{{ old('title') ?? $post->title }}"
                                                type="text" name="title" placeholder="{{ $postType->title }} Title..."
                                                class="form-control main-label" required>
                                        </div>
                                        @if ($postType->has_sub_title == 1)
                                            <div class="form-group">
                                                <p>Sub Title</p>
                                                <label for="t-text" class="sr-only">Sub Title</label>
                                                <input id="sub_title" value="{{ old('sub_title') ?? $post->sub_title }}"
                                                    type="text" name="sub_title" placeholder="Sub Title..."
                                                    class="form-control">
                                            </div>
                                        @endif
                                        <div class="form-group">
                                            ...
                                            <div class="input-group mb-3">
                                                ...
                                                <input  id="slug"
                                                    value="{{ old('title') ?? $post->slug }}" type="text" name="slug"
                                                    placeholder="{{ $postType->title }} Slug..." class="form-control slugfield"
                                                    required>
                                            </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control  basic" name="status">
                                                <option value="Active" @if ($post->status == 'Active') selected @endif>
                                                    Active</option>
                                                <option value="InActive" @if ($post->status == 'InActive') selected @endif>
                                                    InActive</option>
                                            </select>
                                        </div>
                                        ...
                                        <input type="submit" name="txt" value="Update"
                                            class="mt-4 btn btn-primary float-right">
                                    </div>
                                </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input id="post_type_title"
                                                    value="{{ old('img_alt') ?? $post->seoable->img_alt }}"
                                                    type="text" name="img_alt" placeholder="Image Alt Text"
                                                    class="form-control">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input id="post_type_title"
                                                    value="{{ old('img_title') ?? $post->seoable->img_title }}"
                                                    type="text" name="img_title" placeholder="Image Title"
                                                    class="form-control">
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="meta_title"
                                        value="{{ old('meta_description') ?? $post->seoable->meta_title }}"
                                    ...
                                    <select class="form-control tagging" multiple="multiple" name="meta_keyword[]">
                                        @foreach ($keywords as $keyword)
                                            <option value="{{ $keyword }}"
                                                @if ($post->seoable->meta_keyword) @foreach (unserialize($post->seoable->meta_keyword) as $thiskeyword)  {{ $thiskeyword == $keyword ? 'selected' : '' }} @endforeach @endif>
                                                {{ $keyword }}</option>
                                        @endforeach
                                    </select>
                                </div>
...
<script>
    ...
    $(document).on("keyup", ".slugfield", function() {
        var thisitem = $(this);
        var labelText = thisitem.val();
        Text = labelText.toLowerCase();
        Text = Text.replace(/[^a-zA-Z0-9]+/g, '-');
