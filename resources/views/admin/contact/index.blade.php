                            ...
                            <div class="col-sm-6">
                                <nav class="breadcrumb-two" aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active"><a href="{{ route('admin.contact') }}">Contact</a></li>
                                    </ol>
                                </nav>
                            </div>
                            <div class="col-sm-6">
                        ...
                        <table id="datatable" class="table table-hover" style="width:100%">
                            ...
                            @foreach($contacts as $contact)
                                <tr>
                                    ...
                                    <td>{{ $contact->name }} </td>
                                    <td>{{ $contact->email }}<a class="badge badge-success ml-4" href="mailto:{{ $contact->email }}">Send Mail</a> </td>
                                    <td >
                                        {{ $contact->phone ?? 'N/A' }}
                                    </td>
                                    {{-- <td>{{ $contact->subject }}</td> --}}
                                    <td>@if($contact->seen == 1) <span class="badge badge-success">Seen</span> @else <span class="badge badge-danger">Unseen</span> @endif</td>
                                    <td>

                                        <a href="{{ route('admin.contact.show', $contact->id) }}" title="Edit" class="badge badge-primary viewPopUpModal"> <i data-feather="eye"></i></a>
                                        <a href="{{ route('admin.contact.delete', $contact->id) }}" title="Delete" class="badge badge-dark warning confirm"><i data-feather="archive"></i></a>
